...
public class MainFrame extends javax.swing.JFrame {
	...
	Semantico semantico = new Semantico();
        ...
        try {
            ...
            int line = getLineFromPosition(e.getPosition(), textArea.getText());
            terminalTextArea.setText("Erro na linha " + line + " - " + e.getMessage());
	} catch ( SyntaticError e ) {
            int line = getLineFromPosition(e.getPosition(), textArea.getText());
            String errorCause = getErrorCause(textArea.getText(), e.getPosition());
            String treatedErrorCause = treatErrorCause(errorCause);
            terminalTextArea.setText("Erro na linha " + line + " - " + "encontrado " + treatedErrorCause + " " + e.getMessage());		
    ...
    private int getLineFromPosition(int position, String text) {
        int line = 1;
    ...
    private String getErrorCause(String text, int position) {
        String trimmedText = text.trim();
        if (position == trimmedText.length()) {
            return "EOF";
        }
        
        String errorCause = String.valueOf(text.charAt(position));
    ...
    private String treatErrorCause(String errorCause) {
        if(errorCause.charAt(0) == '\"' || errorCause.charAt(0) == '\''){
            errorCause = "constante_string";
        }
